...
(
  ...
  properties:
   {
    ...
    openid:
    {
      type:String,
      value:'',
      observer(newVal,oldVal,changedPath)//监听器，监听openid的变化
      {
        console.log("1openid:"+newVal);
        this.properties.openid=newVal;
        console.log("1openid:"+this.properties.openid);
      }
    }
  },
  ...
  methods: 
  {
    bindtapToCart:function()//点击添加购物车的情况
    {
      //创建订单
      const app = getApp()
      ...
      db.collection('cart').add(
        {
          data://添加的信息
          {
            bookname:this.data.mname,//书籍的名称
            photoPath:this.data.mphotoPath,//书籍的图片
            price:this.data.mprice,//书籍的价格
            id:this.data.midstr,//书籍的id
            number:this.data.mnumber,//书籍的数量
            owner_openid:this.data.openid,//书籍的拥有者的openid
            buyer_openid: app.globalData.user_openid,//书籍的购买者的openid
            ifpaid:false,//书籍是否支付
          }
...
;
    ...
    _getEverything:function()
    {
      ...
      db.collection('cart').where(
        {
          id:this.properties.idstr,//通过书籍的id来查找
        }
      ).get(
        {
          success:res=>//成功找到之后，返回res书籍所有信息
          {
            ...
            this.properties.photoPath=res.data[0].photoPath;
            this.properties.openid=res.data[0].openid;
            ...
            console.log("e1phth:"+this.data.mphotoPath);
            this.setData
            (
              {
                mphotoId:photoId,
            ...
            this.setData(
              {
                mphotoId:photoId,
                mphotoPath:this.properties.photoPath,
                mopenid:this.properties.openid
              }
            )
